...
async def test_exec_tool_uses_scope_project_as_default_cwd(tmp_path: Path) -> None:
    ...
    try:
        result = await tool.execute(
            command='python -c "from pathlib import Path; Path(\'scoped-marker.txt\').write_text(\'ok\')"'
        )
    finally:
        reset_workspace_scope(token)
...
async def test_exec_full_scope_allows_explicit_cwd_outside_project(tmp_path: Path) -> None:
    ...
    try:
        result = await tool.execute(
            command='python -c "from pathlib import Path; Path(\'outside-marker.txt\').write_text(\'ok\')"',
