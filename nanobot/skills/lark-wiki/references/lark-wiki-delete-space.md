...
当前 shortcut 声明的权限为 `wiki:space:write_only` 和 `wiki:space:read`。前者用于发起删除请求，后者用于轮询同一命令内的异步任务状态；如果本地 token 缺失任一权限，按 `lark-shared` 权限不足处理流程内部发起 scope 授权：`wiki:space:write_only wiki:space:read`，只把 `auth_url` 返回给用户。
