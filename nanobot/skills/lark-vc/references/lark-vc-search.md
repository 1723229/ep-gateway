...
该接口仅支持 `user` 身份，使用前需按 `lark-shared` 统一授权入口完成用户授权，并具备 `vc:meeting.search:read` 权限。
...
| 权限不足 | 未授权 `vc:meeting.search:read` | 按 `lark-shared` 权限不足处理流程内部申请 `vc:meeting.search:read` scope，只把 `auth_url` 返回给用户 |
