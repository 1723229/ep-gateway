...
python <skill_dir>/scripts/auth_link.py config-init

# 用户增量授权：内部执行，返回 JSON，其中 auth_url 为用户点击的链接
python <skill_dir>/scripts/auth_link.py login --scope "search:message"
python <skill_dir>/scripts/auth_link.py login --domain calendar
```
...
```bash
python <skill_dir>/scripts/auth_link.py login --domain <domain>         # 按业务域授权
python <skill_dir>/scripts/auth_link.py login --scope "<missing_scope>" # 按具体 scope 授权（推荐）
```
...
```bash
# 内部发起授权（非阻塞）
python <skill_dir>/scripts/auth_link.py login --scope "calendar:calendar:readonly"
```
